...
- Satisfy the risk management requirements of NIST CSF 2.0 (Adaptive tier), NIST 800-53 Rev 5, NIST 800-171 Rev 2, CMMC Level 2, NERC-CIP, and SOX ITGC simultaneously.
...
### 2.2 NIST CSF Function Mapping

NIST CSF 2.0 defines six functions. The CERG-RMF operationalizes all six within the risk cycle:

| CSF Function | CERG-RMF Phase(s) | What CERG Does |
|---|---|---|
...
- Data types by confidentiality, integrity, and availability (CIA) impact: Low, Moderate, or High, per FIPS 199 / NIST 800-60.
...
|---|---|
| **Governance** | Leads the categorization process. Maintains the system categorization register. Ensures regulatory designations (BES Cyber System, CUI scope, SOX scope) are documented and current. Coordinates annual review. |
...
- **Regulatory Scope Declarations** — explicit documentation of which systems are in-scope for NERC-CIP (BES/EACMS/PACS), CMMC (CUI enclave), and SOX ITGC.

### 3.5 Regulatory Alignment — Categorize

| Framework | Categorization Requirement | CERG Satisfies Via |
|---|---|---|
| NIST RMF | Step 1: Categorize the system using FIPS 199 / NIST 800-60 | System Categorization Register; CIA impact ratings per asset |
| NIST 800-53 | RA-2: Security Categorization | Governance-maintained categorization register reviewed annually |
...
| CMMC | All 14 domains require defined CUI scope | CUI enclave declaration maintained by Governance; boundary confirmed by Engineering |
| SOX | ITGC scoping: identify systems that support financial reporting | Governance maintains SOX ITGC system register in coordination with Finance and Internal Audit |
...
|---|---|---|
| **Organizational Baseline** | All systems | NIST 800-53 Rev 5 Moderate; NIST CSF Subcategories |
| **High Impact Overlay** | Systems with any High CIA impact rating | NIST 800-53 High baseline additions |
| **CUI Overlay** | Systems handling Controlled Unclassified Information | NIST 800-171 Rev 2 (110 practices) |
| **BES Overlay** | BES Cyber Systems, EACMs, PACSs | NERC-CIP CIP-002 through CIP-014 |
| **SOX ITGC Overlay** | Systems supporting financial reporting | COSO/ITGC: Change management, access, availability |
| **OT Safety Overlay** | ICS/SCADA with safety implications | IEC 62443 + NIST 800-82 supplemental controls |

> **The Overlay Principle**
>
> Overlays do not restart the control selection process — they add to the organizational baseline. A system in both the CUI scope and SOX scope carries the organizational baseline plus both applicable overlays. Governance tracks the combined control set per system; Engineering implements it. This prevents the "compliance tunnel vision" problem where teams optimize for one framework and inadvertently create gaps in another.
...
- **NIST 800-82** supplemental guidance for OT and ICS system hardening, recognizing that CIS benchmarks may not directly apply to industrial control systems.
...
| **Select** | NIST 800-53 Moderate/High baseline | Apply OT Safety Overlay: IEC 62443 practices; NIST 800-82 supplemental controls; vendor-specific hardening where CIS benchmarks don't apply. |
...
| **NIST CSF 2.0** | GOVERN: Risk strategy, risk appetite, accountability structures | Phase 5 (Authorize) | Governance |
| **NIST CSF 2.0** | IDENTIFY: Asset management, risk assessment, improvement | Phases 1, 4, 6 | Risk + Governance |
| **NIST 800-53** | RA-2, RA-3, RA-5, CA-2, CA-5, CA-7 (Categorization, Risk Assessment, Vuln Monitoring, Control Assessments, POA&M, Continuous Monitoring) | All phases | Risk + Governance |
| **NIST 800-171** | 3.11: Risk Assessment; 3.12: Security Assessment; all documentation requirements | Phases 1, 4, 5, 6 | Risk + Governance |
...
| **CMMC Level 2** | RM.2.141 (vulnerability scanning); CA.2.157 (assessment); all 110 practices documented in SSP and POA&M | Phases 1, 4, 5, 6 | Risk + Governance |
