...
3. **Patching.** The operating system and installed software are patched against the SLAs in [`CERG-PRC-VM-001`](CERG-PRC-VM-001_Vulnerability_Management_Procedure.md). The endpoint reports its patch state to management.
...
5. **CERG feeds, the IR team responds.** Consistent with [`CERG-GOV-OM-001`](CERG-GOV-OM-001_CERG_Operating_Model.md) §3.4, CERG operates the EDR capability and feeds its telemetry and detections to the standing Incident Response team. CERG does not run incident response from this standard.
...
| **Vendor Risk Analyst** | Coordinates transient contractor-device access with [`CERG-PRC-TPRM-001`](CERG-PRC-TPRM-001_Third_Party_and_Supply_Chain_Risk_Procedure.md). |
...
| Vulnerability Management Procedure | [`CERG-PRC-VM-001`](CERG-PRC-VM-001_Vulnerability_Management_Procedure.md) | Endpoint patching SLAs |
| Third-Party and Supply Chain Risk Procedure | [`CERG-PRC-TPRM-001`](CERG-PRC-TPRM-001_Third_Party_and_Supply_Chain_Risk_Procedure.md) | Contractor transient devices |
| Operating Model | [`CERG-GOV-OM-001`](CERG-GOV-OM-001_CERG_Operating_Model.md) | CERG feeds EDR telemetry; the IR team responds |
| Document Catalog and Naming Convention | [`CERG-GOV-CAT-001`](CERG-GOV-CAT-001_Document_Catalog_and_Naming_Convention.md) | Registers this artifact and the `EP` domain |
